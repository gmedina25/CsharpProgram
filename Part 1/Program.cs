...
namespace ConsoleRegisterStudent
{
    class Program
    {
            ...
            do
            {
                ...
                switch (ValidateChoice(choice, firstChoice, secondChoice, thirdChoice, totalCredit))
                {
                    case -1:
                        ...
                        break;
                    case -2:
                        Console.WriteLine("You have already registered for this {0} course.", ChoiceToCourse(choice)); //typo on registerd to registered
                        break;
                    case -3:
        ...
        {
            if (choice < 1 || choice > 7) //was choice > 70 changed to choice > 7
                return -1;
            else if (choice == firstChoice || choice == secondChoice || choice == thirdChoice) //changed &&(AND) operators to ||(OR) operators
                return -2;
            else if (totalCredit == 9) // was totalCredit > 9 changed to totalCredit == 9
                return -3;
            return 0; //was return -4 changed to return 0
        }
